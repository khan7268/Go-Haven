...
function BookingPage() {
  ...
  return (
    ...
    <div className="my-8 px-4 max-w-[1200px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold mb-1">{booking.place.title}</h1>
          <AddressLink>{booking.place.address}</AddressLink>
        </div>

        <button
          onClick={handleCancelBooking}
          className="bg-[#F5385D] hover:bg-[#e02649] cursor-pointer text-white px-4 py-2 rounded-lg transition duration-200 w-full sm:w-auto"
        >
          Cancel Booking
        </button>
      </div>
      <div className="bg-gray-200 p-4 sm:p-6 my-6 rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
          <h2 className="text-2xl font-semibold mb-2">Your booking information:</h2>
          <BookingDates booking={booking} />
        </div>
        <div className="bg-[#F5385D] p-5 rounded-2xl text-white text-center">
          <div>Total Price:</div>
          <div className="text-3xl font-semibold">${booking.price}</div>
        </div>
      </div>
      <PlaceGallery place={booking.place} />
      {/* Display CancelBooking modal if showCancelModal is true */}
      {showCancelModal && (
        <CancelBooking
          booking={booking}
      ...
      <div className="relative mt-8 grid gap-6">
        <div>
          <h1 className="text-2xl font-semibold mb-1">
            Room in {booking.place.address}
          </h1>
          <p className="text-lg mb-4">{booking.place.maxGuests} beds. Dedicated Bathroom</p>

          {/* Review Card */}
          <div className="border border-gray-800 rounded-2xl my-6 px-4 py-4 grid grid-cols-1 md:grid-cols-[1fr_2fr_0.7fr] items-center gap-6">
            {/* Guest Info */}
            ...
            <div className="text-center text-lg font-semibold">
              <h2>4.9</h2>
              <div className="grid grid-cols-5 gap-1 mt-1 justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="size-3">
                  <path fillRule="evenodd" d="M8 1.75a.75.75 0 0 1 .692.462l1.41 3.393 3.664.293a.75.75 0 0 1 .428 1.317l-2.791 2.39.853 3.575a.75.75 0 0 1-1.12.814L7.998 12.08l-3.135 1.915a.75.75 0 0 1-1.12-.814l.852-3.574-2.79-2.39a.75.75 0 0 1 .427-1.318l3.663-.293 1.41-3.393A.75.75 0 0 1 8 1.75Z" clipRule="evenodd" />
                </svg>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="size-3">
          ...
          <div className="mb-4 text-sm leading-relaxed">
            <p>Check-in: {booking.place.checkIn}</p>
            <p>Check-out: {booking.place.checkOut}</p>
            <p>Maximum Guests: {booking.place.maxGuests}</p>
          </div>
